...
def _set_branch_lengths(
    ...
    left_clade, left_height = _set_branch_lengths(
        bio_clade.clades[0], taxa_names, clade_to_branch_lengths
    )
    right_clade, right_height = _set_branch_lengths(
    ...
    if left_height <= right_height:
        bio_clade.clades[0].branch_length = min_branch_length + (right_height - left_height)
        ...
        bio_clade.clades[1].branch_length = min_branch_length + (left_height - right_height)

        return clade_bitstring, min_branch_length + left_height


def set_branch_lengths(tree: Tree, branch_lengths: list[float], clades: list[int]):
    clade_to_branch_lengths = dict(zip(clades, branch_lengths))
    _set_branch_lengths(tree.root, get_taxa_names(tree), clade_to_branch_lengths)
